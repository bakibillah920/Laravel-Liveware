        ...
        <div class="col-lg-12">
            <div class="card shadow mt-3">
                ...
                <div class="card-body torrent-list" id="globalModal">
                    ...
                    <form action="{{ route('permissions.update', $permission->id) }}" method="post" id="globalForm" class="form-horizontal pageLoad" enctype="multipart/form-data">
                        @csrf
                        @method('PATCH')
                        ...
                        <div class="row">
                            <div class="col-lg-6">
                                <div class="form-group">
                                    <label class="bmd-label-floating" for="name"> Parent : </label>
                                    <select name="parent" id="parent" class="form-control select2">
                                        <option value="">Select if any</option>
                                        @foreach($permissions as $per)
                                            <option value="{{ $per->id }}" {{ $permission->parent_id == $per->id ? 'selected':'' }}>{{ $per->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="col-lg-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="name" id="name" class="form-control" required="true" autocomplete="nofill" value="{{ $permission->name }}">
                                </div>
                            </div>
                            <div class="col-lg-12">
                                <div class="form-group">
                                    <label class="bmd-label-floating" for="description"> Description : </label>
                                    <textarea name="description" id="description" class="form-control description" required="true">{!! $permission->getDescription() !!}</textarea>
                                </div>
                            </div>
                        </div>
                        <div class="row">
