    ...
    <div class="row">
                ...
                <div class="card-body torrent-list" id="globalModal">
                    ...
                    <form action="{{ route('mail-box.store') }}" method="post" id="globalForm" class="form-horizontal pageLoad" enctype="multipart/form-data">
                        ...
                        <div class="form-group print-error-msg" style="display:none">
                            <a href="{{ route('mail-box.replayMail', [$receiver->id, $replay_id]) }}" id="redirectPath"></a>
                        ...
                        <div class="row">
                            <div class="col-lg-12">
                                <div class="form-group">
                                    ...
                                    <select name="" class="form-control select2" multiple disabled="true">
        ...
        <div class="col-lg-12">
            <div class="card shadow mt-3">
                ...
                <div class="card-body">
                    @foreach($pms as $pm)
                        <div class="{{ $pm->reciever_id == Auth::user()->id ? 'bg-info':''}}">
                            <div class="card shadow {{ !$loop->first ? 'mt-3':'' }}">
                                <div class="card-header"><strong>{{ $pm->subject }}</strong> <div class="float-right">{{ \Carbon\Carbon::parse($pm->created_at)->diffForHumans() }}</div></div>
                                <div class="card-body">{{ $pm->getDescription() }}</div>
                            </div>
                        </div>
                    @endforeach
                </div>
