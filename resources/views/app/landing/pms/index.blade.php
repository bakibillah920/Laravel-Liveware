        ...
        <div class="col-lg-12">
            <div class="card shadow mt-3">
                <div class="card-header">
                    ...
                    <a href="{{ route('outbox') }}" class="float-right mr-2 btn btn-outline-info btn-sm">
                        <i class="zmdi zmdi-mail-send zmdi-hc-lg"></i> Outbox
                    </a>
                    <a href="{{ route('mail-box.readAll') }}" class="float-right mr-2 btn btn-outline-warning btn-sm"
                ...
                <div class="card-body torrent-list" id="globalTable">
                    <table id="datatable" class="table border table-striped table-hover display responsive nowrap" cellspacing="0" width="100%">
                        ...
                        @foreach($pms as $pm)
                            ...
                            if($pm->sender->username == 'System'){ $route = "JavaScript:void(0)"; }else{ $route = route('profile.show', $pm->sender->username ); }
                            ?>
                            <tr data-row="{{ $pm->id }}">
                                <td class="text-center" width="30px">{!! $pm->read_at == '' ? '<i class="zmdi zmdi-album text-primary"></i>':'' !!} {{ $loop->iteration }}</td>
                                <td class="">{!! '<a href="'.$route.'">' .$pm->sender->username .$count.'</a>' .'<br>'. \Carbon\Carbon::parse($pm->created_at)->format('d-M-y - g:i A') !!}</td>
                                <td class="">
                                ...
                                <td style="border: none;">
                                    ...
                                    @if($pm->sender->username == "System")
                                    @else
                                        <a href="{{ route('mail-box.replayMail', [$pm->sender->id, $pm->id]) }}" id="submitBtn" title="Replay PM"
